...
if [ -f /proc/sys/kernel/randomize_va_space ]; then
    ...
    echo "⚠️ ASLR is not supported on this system."
fi
...
if ! lsmod | grep -q "nvidia\|vboxdrv"; then
    ...
    echo "ℹ️ Kernel module loading has been disabled."
else
    echo "⚠️ Kernel module loading is not disabled due to detected external drivers."
fi
